    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          ...
          className="self-start md:self-auto px-6 py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-250 text-sm font-bold rounded-xl border border-slate-250/20 transition-all flex items-center gap-2"
        >
            ...
            <table className="w-full text-right border-collapse text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800/80 text-slate-400 dark:text-slate-555">
                  <th className="py-3.5 px-4 font-bold font-tajawal">الزبون</th>
                  <th className="py-3.5 px-4 font-bold font-tajawal">رقم الفاتورة</th>
                  <th className="py-3.5 px-4 font-bold font-tajawal">المنتج المرتجع</th>
                  <th className="py-3.5 px-4 font-bold font-tajawal">الكمية المستردة</th>
                  <th className="py-3.5 px-4 font-bold font-tajawal">تاريخ الإرجاع</th>
                  <th className="py-3.5 px-4 font-bold font-tajawal">حالة المنتج</th>
                  <th className="py-3.5 px-4 font-bold font-tajawal">الموظف المستلم</th>
                  <th className="py-3.5 px-4 font-bold font-tajawal">ملاحظات الفحص</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50 dark:divide-slate-850">
                {filteredReturns.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/20 transition-all">
                    <td className="py-4.5 px-4 font-bold text-slate-800 dark:text-slate-200">{r.customerName}</td>
                    <td className="py-4.5 px-4 font-black text-brand-600 dark:text-brand-400 font-tajawal">{r.orderNumber}</td>
                    <td className="py-4.5 px-4 font-bold text-slate-700 dark:text-slate-350">
                      {translateCategory(r.category, r.customCategory)}
                    </td>
                    <td className="py-4.5 px-4 font-bold text-slate-700 dark:text-slate-300">{r.quantityReturned} قطع</td>
                    <td className="py-4.5 px-4 font-semibold text-slate-500 dark:text-slate-455 font-tajawal">{formatDate(r.returnDate)}</td>
                    ...
                    <td className="py-4.5 px-4 font-bold text-slate-655 dark:text-slate-350">{r.employeeName}</td>
                    <td className="py-4.5 px-4 font-semibold text-slate-400 dark:text-slate-500 font-tajawal">{r.notes || 'لا يوجد ملاحظات'}</td>
                  </tr>
                ))}
